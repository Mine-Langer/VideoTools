...
int sq_send(SyncQueue* sq, unsigned int stream_idx, SyncQueueFrame frame)
{
    ...
    if (st->frames_sent >= st->frames_max) {
        av_log(sq->logctx, AV_LOG_DEBUG, "sq: %u frames_max %lld reached\n",stream_idx, st->frames_max);
...
static void offset_audio(AVFrame* f, int nb_samples)
{
    ...
    f->duration = av_rescale_q(f->nb_samples, { 1, f->sample_rate }, f->time_base);
    f->pts += av_rescale_q(nb_samples, { 1, f->sample_rate }, f->time_base);
}
...
finish:
    dst->duration = av_rescale_q(nb_samples, { 1, dst->sample_rate }, dst->time_base);
