...
HRESULT ListAVDevice::DS_GetAudioVideoInputDevices(std::vector<std::string>& vectorDevices, std::string deviceType)
{
	GUID guidValue;
	if (deviceType == "v") {
	...
	if (hr == S_OK)
	{
		...
		while (pEnumCat->Next(1, &pMoniker, &cFetched) == S_OK)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					StringCchCopy(FriendlyName, MAX_PATH, varName.bstrVal);
...
std::string ListAVDevice::DS_GetDefaultDevice(std::string type)
{
	if (type == "a") {
...
std::string ListAVDevice::GbkToUtf8(const char* src_str)
{
...
std::string ListAVDevice::Utf8ToGbk(const char* src_str)
{
...
std::string ListAVDevice::unicode2utf8(const WCHAR* uni)
{
