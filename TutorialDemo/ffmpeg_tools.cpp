    ...
    if (do_benchmark_all) {
        ...
        if (fmt) {
            ...
            av_log(NULL, AV_LOG_INFO,
                "bench: %8lld user %8lld sys %8lld real %s \n",
...
void close_output_stream(OutputStream* ost)
{
    ...
    if (ost->sq_idx_encode >= 0)
    {
        SyncQueueFrame frm = {};
        frm.f = nullptr;
        sq_send(of->sq_encode, ost->sq_idx_encode, frm);
    }
}
...
//   FFABS64U(a) ((a) <= 0 ? -(uint64_t)(a) : (uint64_t)(a))
    if (pts <= 0) pts *= -1;
    us = (pts) % AV_TIME_BASE;
    secs = (pts) / AV_TIME_BASE % 60;
    mins = (pts) / AV_TIME_BASE / 60 % 60;
    hours = (pts) / AV_TIME_BASE / 3600;
    hours_sign = (pts < 0) ? "-" : "";
...
int copy_av_subtitle(AVSubtitle* dst, const AVSubtitle* src)
{
    ...
    tmp.start_display_time = src->start_display_time;
    tmp.end_display_time = src->end_display_time;
    tmp.pts = src->pts;
//     AVSubtitle tmp = {
//         .format = src->format,
...
{
    OutputFile* of = output_files[ost->file_index];
    int64_t signal_pts = av_rescale_q(pkt->pts, pkt->time_base, { 1, AV_TIME_BASE });
