...
{
	bool bRet = false;
	uint8_t** convertBuffer = new uint8_t* [m_pCodecCtx->channels];
	if (0 > av_samples_alloc(convertBuffer, nullptr, m_pCodecCtx->channels, frame->nb_samples, m_pCodecCtx->sample_fmt, 0))
		goto ends;

	if (0 > swr_convert(m_pSwrCtx, convertBuffer, frame->nb_samples, (const uint8_t**)frame->extended_data, frame->nb_samples))
		goto ends;

	if (0 > av_audio_fifo_realloc(m_pAudioFifo, av_audio_fifo_size(m_pAudioFifo) + frame->nb_samples))
		goto ends;

	if (0 > av_audio_fifo_write(m_pAudioFifo, (void**)convertBuffer, frame->nb_samples))
		goto ends;
