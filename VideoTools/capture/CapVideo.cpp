...
bool CCapVideo::Init(int posX, int posY, int sWidth, int sHeight)
{
	char szTemp[32] = { 0 };
...
bool CCapVideo::Start(IVideoEvent* pEvt)
{
	...
	m_thread = std::thread(&CCapVideo::OnDecodeFunction, this);

	return true;
}

void CCapVideo::Release()
{
	if (m_pFormatCtx)
	{
...
void CCapVideo::OnDecodeFunction()
{
	AVPacket packet = { 0 };
